## A theoretical "hardcoded" implementation of the MoIST framework. In other words, what if we had a router that was "100%" accurate?
...
class MoE(nn.Module):
    def __init__(self, students, big_class_map):
        super(MoE, self).__init__()
        self.students = nn.ModuleList(students)
        self.big_class_map = big_class_map

    def forward(self, x, targets):
        batch_size = x.size(0)

        # Map targets to their corresponding big classes using `big_class_map`
        big_classes = torch.tensor(
            [self.big_class_map[label.item()] for label in targets],
            device=x.device
        )

        # Initialize outputs tensor
        outputs = torch.zeros(batch_size, self.students[0].network[-1].out_features, device=x.device)

        # Route inputs to the appropriate students
        for student_idx in range(len(self.students)):
            mask = big_classes == student_idx
            if mask.sum() > 0:  # Process only if there are inputs for this student
                inputs = x[mask]
                outputs[mask] = self.students[student_idx](inputs)
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(loader):
            ...
            if i == 0:
                ...
                # Calculate FLOPs differently for MoE vs non-MoE
                flops_per_image = flops_analysis.total() / batch_size if isinstance(model, MoE) else flops_analysis.total()

                end_time = time.time()
                latency = (end_time - start_time) / batch_size

            # Handle MoE model by passing both inputs and targets
            if isinstance(model, MoE):
                outputs = model(inputs, targets)  # MoE requires both inputs and targets
            else:
                outputs = model(inputs)  # Non-MoE models only use inputs
...
def evaluate_moe(model, loader, device, description="MoE"):
    model.eval()
    criterion = nn.CrossEntropyLoss()
    total_loss, correct = 0, 0
    total_samples = 0
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(loader):
            ...
            if i == 0:
                ...
                # Compute FLOPs for the MoE model
                flops_input = inputs[:1].to(device)
                dummy_targets = torch.zeros(flops_input.size(0), dtype=torch.long).to(device)
                flops_analysis = FlopCountAnalysis(model, (flops_input, dummy_targets))
                flops_per_image = flops_analysis.total()

                end_time = time.time()
                latency = (end_time - start_time) / batch_size


            # Pass both inputs and targets to the MoE model
            outputs = model(inputs, targets)  # Explicitly pass targets
            loss = criterion(outputs, targets)
...
def train_moe(moe_model, train_loader, optimizer, criterion, device, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in train_loader:
            ...
            # Forward pass with hard routing
            outputs = moe_model(inputs, targets)  # Pass inputs and targets for hard routing
            loss = criterion(outputs, targets)
...
def main():
    ...
    # Initialize MoE with hard routing
    moe_model = MoE(students, big_class_map).to(device)

    # Train the MoE model
    print("\nTraining Mixture of Experts (MoE) with Hard Routing:")
    ...
    print("\nEvaluating Mixture of Experts (MoE) with Hard Routing:")
    evaluate_moe(moe_model, test_loader, device, description="MoE (Hard Routing)")
