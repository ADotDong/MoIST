...
class TransformerRouter(nn.Module):
    def __init__(self, num_students, d_model=64, nhead=4, num_layers=2):
        super(TransformerRouter, self).__init__()
        self.conv = nn.Conv2d(3, d_model, kernel_size=3, padding=1)
        self.transformer = nn.TransformerEncoder(
        nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead), num_layers=num_layers)
        self.fc = nn.Linear(d_model, num_students)
    ...
    def forward(self, x):
        ...
        assert x.dim() == 4, f"Input dimension should be 4, but got {x.dim()}"
...
def main():
    ...
    router = TransformerRouter(num_students=config.num_students, d_model=64, nhead=4, num_layers=2).to(device)
